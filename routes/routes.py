...
from utils.logging_setup import app_logger, error_logger
...
def setup_routes(app, data_store):
    ...
    def query_and_add_list():
        try:
            ...
            app_logger.info(f"接收到的資料：{data}")
            app_logger.info(f"處理的網址或名稱或ID：{url_or_name_or_id}")
            
            if url_or_name_or_id is None:
                raise ValueError("沒有值: 'urlOrNameOrId'")
            
            if not url_or_name_or_id:
                app_logger.warning("未提供網址或名稱或ID")
            ...
            if existing_item:
                ...
                app_logger.info(f"嘗試取得直播流：{live_stream_url}，狀態:{status}")
                existing_item["live_stream_url"] = live_stream_url
                existing_item["status"] = status
                app_logger.info(f"已存在的項目：{existing_item}")
            ...
            if "chaturbate.com" in url_or_name_or_id:
                new_item = process_url_or_name(data_store, url_or_name_or_id, data)
            else:
                test_url = f"https://chaturbate.com/{url_or_name_or_id}/"
                new_item = process_url_or_name(data_store, test_url, data, url_or_name_or_id)
            
            if new_item:
                update_data_store_and_file(data_store, new_item)
                return jsonify(new_item), 201
            
            app_logger.warning("未找到頁面")
    ...
    def delete_list():
        ...
        app_logger.info(f"刪除的網址或名稱或ID：{url_or_name_or_id}")
        ...
        for item in data_store["live_list"]:
            if item.get("id") == url_or_name_or_id or item.get("url") == url_or_name_or_id or item.get("name") == url_or_name_or_id:
                ...
                app_logger.info(f"已刪除的項目：{item}")
                return jsonify({"message": "Item deleted"}), 200
        app_logger.warning("未找到項目")
    ...
    def update_list_status():
        ...
        app_logger.info(f"更新的網址或名稱或ID：{url_or_name_or_id}")
        
        for item in data_store["live_list"]:
            if item.get("id") == url_or_name_or_id or item.get("url") == url_or_name_or_id or item.get("name") == url_or_name_or_id:
                app_logger.info(f"更新的項目：{item}")
                ...
                if item["isFavorite"]:
                    if item["id"] not in data_store["favorites"]:
                        ...
                        app_logger.info(f'數量:{len(data_store["favorites"])}')
                else:
                ...
                if item["autoRecord"]:
                    if item["id"] not in data_store["auto_record"]:
                        ...
                        app_logger.info(f'數量:{len(data_store["auto_record"])}')
                else:
                    if item["id"] in data_store["auto_record"]:
                        data_store["auto_record"].remove(item["id"])
                        app_logger.info(f'數量:{len(data_store["auto_record"])}')

                app_logger.info(f"更新後資料：{len(data_store['live_list'])}")
                write_json_file(data_store)
    ...
    def get_list():
        try:
            ...
            app_logger.info(f'searchQuery: {searchQuery}')
            ...
            if searchQuery:
                ...
                app_logger.info(f'search_query_lower: {search_query_lower}')
                filtered_list = [
            ...
            if not paginated_list:
                app_logger.info('未初始化完成或沒有資料')
            else:
                app_logger.info(f"查詢成功：取得{len(paginated_list)}筆資料")
    ...
    def recording_control():
        ...
        app_logger.info(f"錄製控制 - ID：{recording_id}，操作類型：{option_type}")

        for item in data_store["live_list"]:
            if item["id"] == recording_id:
                if option_type == "start":
                    if item["url"] in data_store["online"]:
                        app_logger.info(f"已經在錄製中：{item['url']}")
